...
const MachineMasterPage = (props) => {
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("machineMaster").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  return (
    <div className="mt-5" style={{ minHeight: "calc(90vh - 9rem)" }}>
      ...
      <AreYouSureDialog
        ...
        onYes={() => deleteRow()}
      />
      <MachineMasterEditDialogComponent
