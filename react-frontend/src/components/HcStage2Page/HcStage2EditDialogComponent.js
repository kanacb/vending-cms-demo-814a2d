...
const BodyStickerOptions = BodyStickerArray.map((x) => ({
  name: x,
  value: x,
})).sort((a, b) =>
  a.name.localeCompare(b.name, undefined, {
...
const ChuteOptions = ChuteArray.map((x) => ({ name: x, value: x })).sort(
  (a, b) =>
    a.name.localeCompare(b.name, undefined, {
      numeric: true,
      sensitivity: "base",
...
const TubeOptions = TubeArray.map((x) => ({ name: x, value: x })).sort((a, b) =>
  a.name.localeCompare(b.name, undefined, {
    numeric: true,
    sensitivity: "base",
  }),
...
const ValveOptions = ValveArray.map((x) => ({ name: x, value: x })).sort(
  (a, b) =>
    a.name.localeCompare(b.name, undefined, {
      numeric: true,
      sensitivity: "base",
...
const IceMakerOptions = IceMakerArray.map((x) => ({ name: x, value: x })).sort(
  (a, b) =>
    a.name.localeCompare(b.name, undefined, {
      numeric: true,
      sensitivity: "base",
...
const HcStage2CreateDialogComponent = (props) => {
  ...
  const RefNoOptions = RefNo.map((elem) => ({
    name: elem.name,
    value: elem.value,
  })).sort((a, b) =>
    a.name.localeCompare(b.name, undefined, {
  ...
  return (
    ...
    >
      <div
        ...
        style={{ maxWidth: "55vw" }}
        role="hcStage2-edit-dialog-component"
      >
        <div>
          <p className="m-0">RefNo:</p>
          <Dropdown
            id="RefNo"
            value={_entity?.RefNo?._id}
            options={RefNoOptions}
            optionLabel="name"
            optionValue="value"
            onChange={(e) => setValByKey("RefNo", { _id: e.value })}
          />
        </div>
