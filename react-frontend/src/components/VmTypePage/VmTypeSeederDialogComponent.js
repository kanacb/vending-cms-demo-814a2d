...
import seederData from "./vmType.seeder.json";
...
const VmTypeSeederDialogComponent = (props) => {
  ...
  const onRun = async () => {
    ...
    try {
      for (let i = 0; i < seederData.length; i++) {
        ...
        const res = await client.service("vmType").create(_data);
        results = [...results, res];
      }
      props.onHide();
      props.alert({
...
export default connect(mapState, mapDispatch)(VmTypeSeederDialogComponent);
