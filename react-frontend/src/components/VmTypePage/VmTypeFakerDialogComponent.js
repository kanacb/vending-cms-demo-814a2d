...
import vmTypeFakerFactory from "./vmTypeFakerFactory";
...
const VmTypeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = vmTypeFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("vmType").create(elem));
...
export default connect(mapState, mapDispatch)(VmTypeFakerDialogComponent);
