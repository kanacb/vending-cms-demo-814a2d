...
const BreakdownDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    ...
    const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.opsCentreId?.name}</p>
    ...
    const inputTextareaTemplate4 = (rowData, { rowIndex }) => <InputTextarea rows={5} cols={30} value={rowData.reasonForBreakdown} autoResize />;
    const inputTextareaTemplate5 = (rowData, { rowIndex }) => <InputTextarea rows={5} cols={30} value={rowData.technicianRemark} autoResize />;
    const pTemplate7 = (rowData, { rowIndex }) => <p >{rowData.machineId?.vmcode}</p>
    const pTemplate8 = (rowData, { rowIndex }) => <p >{rowData.technicianId?.name}</p>
    ...
    return (
        ...
        currentPageReportTemplate="{first} to {last} of {totalRecords}" paginatorLeft={paginatorLeft} paginatorRight={paginatorRight} rowClassName="cursor-pointer">
            <Column field="opsCentreId" header="opsCentreId" body={pTemplate0} style={{ minWidth: "8rem" }} />
            <Column field="locationId" header="locationId" body={pTemplate1} style={{ minWidth: "8rem" }} />
            <Column field="visitDate" header="Visited Date" body={pTemplate2} sortable style={{ minWidth: "8rem" }} />
            <Column field="reportDate" header="Reported Date" body={pTemplate3} sortable style={{ minWidth: "8rem" }} />
            <Column field="reasonForBreakdown" header="Reason For Breakdown" body={inputTextareaTemplate4} style={{ minWidth: "8rem" }} />
            <Column field="technicianRemark" header="Technician Remarks" body={inputTextareaTemplate5} style={{ minWidth: "8rem" }} />
            <Column field="machineId" header="Machine" body={pTemplate7} style={{ minWidth: "8rem" }} />
            <Column field="technicianId" header="Technician" body={pTemplate8} sortable style={{ minWidth: "8rem" }} />
