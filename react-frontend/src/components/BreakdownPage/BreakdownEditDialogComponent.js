...
const ticketTypeOptions = ticketTypeArray
...
const BreakdownCreateDialogComponent = (props) => {
  ...
  useEffect(() => {
    ...
    client
      ...
      .then((res) => {
        setMachineId(
          res.data.map((e) => ({ name: e["vmcode"], value: e._id })),
        );
      })
      .catch((error) => {
        console.log({ error });
  ...
  const opsCentreIdOptions = opsCentreId
    .map((elem) => ({ name: elem.name, value: elem.value }))
    .sort((a, b) =>
      a.name.localeCompare(b.name, undefined, {
        numeric: true,
        sensitivity: "base",
  ...
  const locationIdOptions = locationId
    .map((elem) => ({ name: elem.name, value: elem.value }))
    .sort((a, b) =>
      a.name.localeCompare(b.name, undefined, {
        numeric: true,
  ...
  const machineIdOptions = machineId
    .map((elem) => ({ name: elem.name, value: elem.value }))
    .sort((a, b) =>
      a.name.localeCompare(b.name, undefined, {
        numeric: true,
  ...
  const technicianIdOptions = technicianId
    .map((elem) => ({ name: elem.name, value: elem.value }))
    .sort((a, b) =>
      a.name.localeCompare(b.name, undefined, {
        numeric: true,
  ...
  return (
    ...
    >
      <div
        ...
        style={{ maxWidth: "55vw" }}
        role="breakdown-edit-dialog-component"
      >
        <div>
          <p className="m-0">Ops Centre:</p>
          <Dropdown
            id="opsCentreId"
            value={_entity?.opsCentreId?._id}
        ...
        <div>
          ...
          <Dropdown
            id="technicianId"
            value={_entity?.technicianId?._id}
