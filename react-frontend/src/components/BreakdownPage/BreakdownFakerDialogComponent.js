...
import breakdownFakerFactory from "./breakdownFakerFactory";
...
const BreakdownFakerDialogComponent = (props) => {
  ...
  useEffect(() => {
    ...
    client
      ...
      .then((data) => {
        setlocationIdIds(data.data);
      });
    client
      .service("machineMaster")
      .find({ query: { $select: ["_id", "vmcode"] } })
      .then((data) => {
  ...
  const onRun = async () => {
    let fakeData = breakdownFakerFactory(
      fakerCount,
      locationIdIds,
      machineIdIds,
      technicianIdIds,
    ...
    const promises = fakeData.map((elem) =>
      client.service("breakdown").create(elem),
    );
...
export default connect(mapState, mapDispatch)(BreakdownFakerDialogComponent);
