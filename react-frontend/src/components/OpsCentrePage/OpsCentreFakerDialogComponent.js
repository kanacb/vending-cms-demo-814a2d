...
import opsCentreFakerFactory from "./opsCentreFakerFactory";
...
const OpsCentreFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = opsCentreFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("opsCentre").create(elem));
...
export default connect(mapState, mapDispatch)(OpsCentreFakerDialogComponent);
