...
import seederData from "./opsCentre.seeder.json";
...
const OpsCentreSeederDialogComponent = (props) => {
  ...
  const onRun = async () => {
    ...
    try {
      for (let i = 0; i < seederData.length; i++) {
        ...
        const res = await client.service("opsCentre").create(_data);
        results = [...results, res];
      }
      props.onHide();
      props.alert({
...
export default connect(mapState, mapDispatch)(OpsCentreSeederDialogComponent);
