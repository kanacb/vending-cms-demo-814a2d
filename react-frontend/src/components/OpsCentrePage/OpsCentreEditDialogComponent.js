...
const OpsCentreCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        const result = await client.service("opsCentre").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info opsCentre updated successfully" });
    ...
    return (
        <Dialog header="Edit Ops Centre" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="opsCentre-edit-dialog-component">
            ...
            <div className="col-12 md:col-6 field mt-5">
                <span className="p-float-label">
                    <InputText id="code" type="text" value={_entity?.code} onChange={(e) => setValByKey("code", e.target.value)}  />
...
export default connect(mapState, mapDispatch)(OpsCentreCreateDialogComponent);
